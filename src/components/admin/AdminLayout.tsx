    ...
    <div className="min-h-screen bg-slate-100 flex flex-col md:flex-row font-sans">
      ...
      <div className="md:hidden bg-navy-500 text-white p-4 flex items-center justify-between sticky top-0 z-50 shadow-md">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-gold-500 text-navy-900 font-bold flex items-center justify-center">
            <Shield className="w-4 h-4" />
          </div>
          <div>
            <h2 className="font-bold text-white text-xs leading-none">STARLIGHT CMS</h2>
            <span className="text-[9px] font-bold text-gold-500 uppercase tracking-wider">{userRole}</span>
          </div>
        </div>

        <button
          onClick={() => setIsMobileNavOpen(!isMobileNavOpen)}
          className="p-2 rounded-lg bg-navy-600 text-white focus:outline-none"
        >
      ...
      `}>
        <div className="space-y-6">
          ...
          {/* Logo & Branding */}
          <div className="flex items-center justify-between border-b border-navy-600 pb-5">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl bg-gold-500 text-navy-900 font-bold flex items-center justify-center">
                <Shield className="w-5 h-5" />
              </div>
              <div>
                <h2 className="font-bold text-white text-sm leading-none">STARLIGHT CMS</h2>
                <span className="text-[10px] font-bold text-gold-500 uppercase tracking-widest">DEO MEDICE ADMIN</span>
              </div>
            </div>
          ...
          {/* Navigation Links */}
          <nav className="space-y-1 overflow-y-auto max-h-[calc(100vh-250px)] scrollbar-thin pr-1">
        ...
        <div className="pt-4 border-t border-navy-600 space-y-3">
          <div className="px-1">
            <span className="text-[10px] text-slate-400 block font-medium uppercase tracking-wider">Logged in user</span>
            <span className="text-xs font-bold text-white block truncate">{user?.name || 'Administrator'}</span>
            <span className="text-[11px] text-gold-500 font-semibold">{userRole}</span>
          </div>
          ...
          <div className="space-y-2">
            <Link
              to="/"
              target="_blank"
              className="w-full flex items-center justify-center gap-1.5 py-2 rounded-lg bg-navy-600 text-slate-200 text-xs font-bold hover:bg-navy-700 transition-colors"
            >
              <ExternalLink className="w-3.5 h-3.5 text-gold-500" /> View Public Site
            </Link>

            <button
              onClick={handleLogout}
              className="w-full flex items-center justify-center gap-1.5 py-2 rounded-lg border border-red-500/30 text-red-400 text-xs font-bold hover:bg-red-500/10 transition-colors"
            >
